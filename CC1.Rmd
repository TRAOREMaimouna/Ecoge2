#Traore Maimouna Batoma
---
title: "R Notebook"
output: github_document
---
#Etape de l'installation des packages    
```{r}
library("knitr")
library("BiocStyle")
...
#Definition de la variable path
````{r}
set.seed(100)
...
#Lecture des 2 premiers forward
```{r}
plotQualityProfile(fnFs[1:2])
```

#?oms de fichiers pour les fichiers fastq.gz filtrés
```{r}
...
#Filtrage des lectures avant et arrière
```{r}
...
#Erreur dans les forward
```{r}
errF <- learnErrors(filtFs, multithread=TRUE)
```

#Erreur dans les reverses
```{r}
...
#Construction de table de séquence et suppression des chimères
```{r}
...
#Contruction d'arbre phylogenetique
```{r}
...
#Installation de packages pour continuer la suite
```{r}
...
#Examination de l’analyse des coordonnées principales (PCoA) avec la dissimilarité de Bray-Curtis sur la distance Unifrac pondérée
```{r}
